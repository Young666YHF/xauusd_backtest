...
for i in range(min(len(trades_fixed), len(result.trades))):
    ...
    print(f"{i+1:<3} {fixed_time:<20} {mart_time:<20} "
...
for t in result.trades[:20]:
    ...
    if abs(t.entry_price - expected_price) > 0.1:
        price_errors += 1
        print(f"  价格错误: {t.entry_time} | 入场价: {t.entry_price:.2f} | 预期: {expected_price:.2f}")
...
for t in result.trades[:15]:
    expected_size = params['position_size'] * (params['martingale_multiplier'] ** martingale_step)
...
for s in expected_sizes[:10]:
    ...
    print(f"{s['time'].strftime('%Y/%m/%d %H:%M'):<18} {s['expected_step']:>8} "
