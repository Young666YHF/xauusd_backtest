...
df["BBW"] = (
    (bb_middle + params["bb_std"] * bb_std - (bb_middle - params["bb_std"] * bb_std))
    / bb_middle
    * 100
)
...
for i in range(warmup, len(df)):
    ...
    if position == "long" and sma_bearish_cross:
        ...
        pnl = (exit_price - entry_price) * params["contract_size"] * params[
            "position_size"
        ] - params["spread_per_ounce"] * params["contract_size"] * params[
            "position_size"
        ]
        py_trades.append(
            {
        ...
        exit_price = open_price + slippage
        pnl = (entry_price - exit_price) * params["contract_size"] * params[
            "position_size"
        ] - params["spread_per_ounce"] * params["contract_size"] * params[
            "position_size"
        ]
...
for t in result.trades[:5]:
    print(
        f"  {t.entry_time.strftime('%Y/%m/%d %H:%M')} | {t.direction.name:5} | 入场: {t.entry_price:.2f} | PnL: ${t.pnl:.2f}"
    )
