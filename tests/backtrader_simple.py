...
class SimpleBBWStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            sma_bearish_cross = (prev2_sma_s >= prev2_sma_m) and (
                prev_sma_s < prev_sma_m
            )
            sma_bullish_cross = (prev2_sma_s <= prev2_sma_m) and (
                prev_sma_s > prev_sma_m
            )
...
def main():
    ...
    for i, s in enumerate(strategy.signals[:10]):
        print(
            f"{i+1}. {s['time'].strftime('%Y/%m/%d %H:%M')} | {s['type']} | "
    ...
    for i, s in enumerate(py_signals[:10]):
        print(
            f"{i+1}. {s['time'].strftime('%Y/%m/%d %H:%M')} | {s['type']} | 入场价: {s['price']:.2f}"
        )
