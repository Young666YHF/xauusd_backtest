...
def create_is_objective_v2(is_df, is_tick_df, bar_idx_to_ticks):
    import optuna
    from run_trend_angle_two_stage import calculate_strategy_indicators, run_tick_backtest, calculate_fitness_is
    
    def objective(trial: optuna.Trial) -> float:
        params = {
            'sma_period': trial.suggest_int('sma_period', 10, 20),  # 短周期
            'angle_threshold': trial.suggest_float('angle_threshold', 1.5, 3.5),  # 低阈值
            'risk_reward_ratio': trial.suggest_float('risk_reward_ratio', 1.0, 2.5),
            'breakout_lookback': trial.suggest_int('breakout_lookback', 2, 4),
            'trailing_stop_atr': trial.suggest_float('trailing_stop_atr', 0.5, 2.0),
            'use_fixed_exit': True,
        }
        try:
...
main_module.create_is_objective = create_is_objective_v2
