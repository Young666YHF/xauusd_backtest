...
print(f"\n分析前10笔交易的入场价格差异:")
print("-" * 100)
print(f"{'时间':<20} {'方向':<6} {'Open价格':>10} {'入场价':>10} {'差异':>8} {'原因':<20}")
print("-" * 100)
...
for t in result.trades[:10]:
    ...
    print(f"{t.entry_time.strftime('%Y/%m/%d %H:%M'):<20} {t.direction.name:<6} "
...
for t in result.trades[:5]:
    ...
    print(f"  {t.entry_time.strftime('%H:%M')} | {t.direction.name} | "
...
print(f"平均每笔滑点成本: ${(total_pnl_no_slippage - total_pnl_with_slippage) / len(result.trades):.2f}")
