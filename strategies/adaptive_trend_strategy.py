...
class AdaptiveTrendMartingaleStrategy(BaseStrategy):
    ...
    def _check_entry_conditions(
        self, prev_bar: pd.Series, current_price: float, atr: float
        ...
        if (
            ...
            and close < ema_trend
            and adx_ok
            and rsi > self.params["rsi_lower"]
            and close < ema_fast
        ):
            return (
                True,
                TradeDirection.SHORT,
                f"Short: EMA↓ ADX={adx:.1f} RSI={rsi:.1f}",
            )
    ...
    def generate_signal(
        ...
        if self.current_position == TradeDirection.LONG:
            ...
            if current_idx >= 2:
                ...
                # 死叉平多
                if (
                    not pd.isna(ema_fast_prev2)
                    and not pd.isna(ema_slow_prev2)
                    and ema_fast_prev2 >= ema_slow_prev2
                    and ema_fast_prev < ema_slow_prev
                ):
                    # 检查是否可以反向开空
                    can_entry, direction, reason = self._check_entry_conditions(
                        prev_bar, current_open, atr
                    )
        ...
        if signal is None and self.current_position is None:
            ...
            if can_entry and direction:
                ...
                signal = self._create_entry_signal(
                    timestamp, direction, current_open, sl, tp, reason, current_idx
                )
